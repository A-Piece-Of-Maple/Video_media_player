...
class Ui_Widget(object):
    ...
    #     img = cv2.flip(img, 1)#画面翻转
    #     img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)#数据格式转换
...
def detect(opt, save_img=False):
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # 逐个帧检测
            ...
            if view_img:
                # cv2.imshow(p, im0)
                im0_image = cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)  ##cv读进来是BGR形式的，需要转换成RGB形式

                im0_image = QtGui.QImage(im0_image[:], im0_image.shape[1], im0_image.shape[0], im0_image.shape[1]*3, QtGui.QImage.Format_RGB888)  # 数据格式转换

                ui.label.setPixmap(QtGui.QPixmap.fromImage(im0_image))
